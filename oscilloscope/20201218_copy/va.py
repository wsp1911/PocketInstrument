...
class AppWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        # self.fig2 = Myplot(width=5, height=5, dpi=100)
        ...
        # self.gridlayout2 = QGridLayout(self.FreqQGB)
    ...
    #     self.measure(signal[0][self.dispStart : self.dispStart + self.dispLen])
    #     self.measure(signal[1][self.dispStart : self.dispStart + self.dispLen])

    def plot_fig(self):
        # if self.FIGPLOTING:
        #     return
        # self.FIGPLOTING = True
        self.fig.ax_sig.cla()
        ...
        # if not self.RUNNING:
        #     self.fig.ax_sig.set_xlim(
        #         (0, self.T[int(self.SECTION / self.tZoomInput.value()) - 1])
        #     )
        # else:
        #     self.fig.ax_sig.set_xlim((0, self.T[self.dispLen - 1]))
    ...
    def byte2num(self, data, gain=20):
        signal = [np.zeros(self.CHUNK), np.zeros(self.CHUNK)]
        for i in range(self.CHUNK):
            signal[0][i] = data[4 * i + 1] * 2 ** 8 + data[4 * i]
            signal[1][i] = data[4 * i + 3] * 2 ** 8 + data[4 * i + 2]
        signal[0] /= 2.0 ** 15
        signal[1] /= 2.0 ** 15

        signal[0] = gain * (
            (signal[0] <= 1) * signal[0] + (signal[0] > 1) * (signal[0] - 2)
        )
        signal[1] = gain * (
            (signal[1] <= 1) * signal[1] + (signal[1] > 1) * (signal[1] - 2)
        )

        if self.MISS:
            self.signal[0] = np.r_[signal[0], self.signal[0]]
            self.signal[1] = np.r_[signal[1], self.signal[1]]
        else:
            self.signal[0] = signal[0]
            self.signal[1] = signal[1]

    def genDebugData(self):
        shift = np.random.randint(-self.CHUNK // 2, self.CHUNK // 2)
        # shift = 0
        amp = 0.2
        # freq = 100
        freq = 100
        y0 = amp * np.sin(
            2 * np.pi * freq * (np.arange(self.CHUNK) + shift) / self.RATE
        )
        y1 = amp * np.cos(
            2 * np.pi * freq * (np.arange(self.CHUNK) + shift) / self.RATE
        )
        # t = np.arange(self.CHUNK)
        # N = self.RATE // freq
        # t1 = t % N
        # y0 = amp * (t1 < N / 2) - amp * (t1 >= N / 2)
        # y1 = amp * (t1 < N / 3) - amp * (t1 >= N / 3)

        y0 = (y0 >= 0) * y0 + (y0 < 0) * (y0 + 2)

        low8 = 0x0000_00FF
        high8 = 0x0000_FF00

        y0 = (y0 * 2.0 ** 15).astype(np.int16)
        y1 = (y1 * 2.0 ** 15).astype(np.int16)

        data = []
        for i in range(self.CHUNK):
            data.append(y0[i] & low8)
            data.append((y0[i] & high8) >> 8)
            data.append(y1[i] & low8)
            data.append((y1[i] & high8) >> 8)
    ...
    def display(self, filepath=None):
        ...
        while True:
            if self.RUNNING:
                ...
                self.byte2num(data)
            if not self.MISS and self.dispStart >= self.CHUNK:
                self.dispStart %= self.CHUNK
            if self.REFRESH:
                self.plot_fig()
                self.REFRESH = False
            while self.RUNNING and self.dispStart < len(self.signal[0]):
                if self.MISS:
                    if self.TRIGGERED:  # 说明在第一段中找到了触发点，再补上一段一定可以显示
                        self.plot_fig()
                        L2del = len(self.signal[0]) - self.CHUNK
                        self.dispStart = self.dispStart + self.dispLen - L2del
                        ...
                        self.trigger()
                        self.TRIGGERED = True
                        if self.dispStart + self.dispLen > 2 * self.CHUNK:
                            break
                        self.plot_fig()
                        self.dispStart = self.dispStart + self.dispLen - self.CHUNK
