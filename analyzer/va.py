...
class AppWindow(QMainWindow, Ui_MainWindow):
    ...
    def genDebugData(self):
        ...
        amp = 0.2
        # freq = 100
        freq = 1 / (self.CHUNK / self.RATE / 10.5)
        y0 = amp * np.sin(
        ...
        for i in range(self.CHUNK):
            self.raw_data[i * 4] = y0[i] & low8
            self.raw_data[i * 4 + 1] = (y0[i] & high8) >> 8
            self.raw_data[i * 4 + 2] = y1[i] & low8
            self.raw_data[i * 4 + 3] = (y1[i] & high8) >> 8
